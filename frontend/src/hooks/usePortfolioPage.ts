...
import type { ChartPoint } from "../components/common/PortfolioAreaChart";
...
export function usePortfolioPage(keycloak: Keycloak) {
  ...
  const [perfPeriod, setPerfPeriodState] = useState<string | null>(null);
  const userOverrodePeriodRef = useRef(false);
  const setPerfPeriod = useCallback((period: string) => {
    userOverrodePeriodRef.current = true;
    setPerfPeriodState(period);
  }, []);
  const [allocView, setAllocView] = useState<AllocationView>("symbol");
  ...
  const allocData = useMemo<AllocationItem[]>(() => {
    const usdtry = marketData.find((item) => item.symbol === "USDTRY");
    const usdRate = usdtry?.last ?? 35.0;
  ...
  const openSellModal = useCallback((position: Position) => {
...
function getSummaryAllocation(summaryDetail: PortfolioSummaryDetail, allocView: AllocationView, usdRate: number): AllocationItem[] {
  if (allocView === "symbol") {
...
function mapAllocation(allocationMap: Record<string, number>): AllocationItem[] {
  return Object.entries(allocationMap)
    .map(([name, value]) => ({ name, value: Math.round(value) }))
    .filter((item) => item.value > 0);
}

function convertFallbackPrice(symbol: string, price: number, type: string | undefined, usdRate: number) {
...
function getMarketLabel(type: string, currency: string) {
...
function getFallbackTypeLabel(symbol: string, type: string | undefined) {
...
function getFallbackMarketLabel(symbol: string, type: string | undefined) {
  if (type === "CRYPTO") return "Kripto";
  if (type === "FUND") return "Fon";
  if (type === "FX") return "Döviz";
  if (type === "COMMODITY") return "Emtia";
...
function isBistLikeSymbol(symbol: string) {
