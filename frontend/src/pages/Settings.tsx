...
type Props = { keycloak: Keycloak; theme: Theme; onThemeChange: (t: Theme) => void };
type NotifKey = "transactions" | "budget" | "investments" | "marketing" | "push" | "security";

const NOTIF_ITEMS: { key: NotifKey; label: string; desc: string; defaultOn: boolean }[] = [
...
export default function Settings({ keycloak, theme, onThemeChange }: Props) {
  const parsed: any = keycloak.tokenParsed;
  const [firstName, setFirstName] = useState<string>(parsed?.given_name ?? parsed?.name?.split(" ")[0] ?? "");
  const [lastName,  setLastName]  = useState<string>(parsed?.family_name ?? parsed?.name?.split(" ").slice(1).join(" ") ?? "");
  const [email,     setEmail]     = useState<string>(parsed?.email ?? "");
  ...
  function toggleNotif(key: NotifKey) { setNotifs((prev) => ({ ...prev, [key]: !prev[key] })); }
  function handleTheme(t: Theme) { onThemeChange(t); applyTheme(t); }
          ...
          <div style={s.card}>
            ...
            <div style={s.themeRow}>
              {(["light", "dark"] as Theme[]).map((t) => {
                const labels: Record<string, string> = { light: "Acik", dark: "Koyu" };
                const icons:  Record<string, string> = { light: "☀️",   dark: "🌙" };
                return (
