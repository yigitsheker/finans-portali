...
export function PortfolioAreaChart({ data, isIntraday = false, height = 200 }: PortfolioAreaChartProps) {
  ...
  useEffect(() => {
    ...
    const normalized = data.map((p) => {
      ...
      if (p.time.includes("T") || p.time.includes(" ")) {
        return { time: Math.floor(new Date(p.time).getTime() / 1000) as unknown as string, value: p.value };
      }
    ...
    seriesRef.current.setData(normalized as any);
