...
try {
    ...
    Write-Host '  Wait until the bottom-left K8s icon turns green, then re-run.'
    exit 1
}
...
# you'd copy these, edit values, and apply manually — see README. For pure
# local dev the example defaults are fine to round-trip with.
$secretFiles = @(
...
Write-Host '  Keycloak admin → http://localhost:8090/admin   (use the password from keycloak-secret)'
